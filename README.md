...
ファーストビューでは、2台のスマートフォン間でQRを使って、その場で情報を直接手渡す体験を主役にしています。Hero直下でQHandの概念を説明し、その後にプロフィールを具体例として掲載します。プロフィール作成や名刺管理ではなく、対面での受け渡しが第一印象になる構成です。

**今回の紹介はオンライン版のみです。** LPでは、開始QRでつながる → 双方の受信準備を確かめる → 暗号化された本文の最終QRで手渡す、という3段階で紹介します。通常版には両端末で比較する確認コードがないため、架空の確認コード画面は掲載しません。オフライン版の数字照合や受信者公開鍵QRの読み返しも含めません。
...
| 開始QR・受信準備・最終QR         | `frontend/src/pages/SenderHandoffPage.tsx`、`ReceiverStartPage.tsx`、`components/HandoffSteps.tsx` を元に再構成 |
...
プロフィール項目の選択、受け渡しデモ、受け渡し完了には、738×1314pxの画面画像を配置しています。最初の受信側標準カメラだけは、同じサイズでQR読み取り場面を描いたSVGを使用します。公開アプリでの撮影には説明用の一時データだけを使用し、実在する個人情報を含めません。

Heroの受け渡しイメージは、2台のスマートフォン、QR、暗号化された情報の移動をHTMLとCSSで構成しています。受け渡しデモは送信側・受信側の実画面と、標準カメラを示す説明用SVGを切り替えます。LP自体は受け渡しAPIやカメラを使いません。

## 操作と検証

- プロフィール項目の選択結果は、実画面スクリーンショットとLP上の説明を並べて示します。
